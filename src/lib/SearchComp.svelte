	...
	<div id="search-component">
		<form id="search-bar-div">
			<input
				...
				id="search-bar"
				type="text"
				placeholder={`Search with ${searchEngineChosen}`}
			/>
		</form>
		<div
			id="search-button"
			...
			<svg
				on:click={() => {
					if (typeof searchVal !== "undefined") {
						let query =
							searchEngineList[
								searchEngineList.findIndex(
									(x) => x.name == searchEngineChosen
								)
							].query;
						window.location.href = query + searchVal;
					}
...
<style>
	...
	#Baidu-menu-icon {
		width: 23px;
		filter: brightness(0) saturate(1) invert(28%) sepia(52%) saturate(2955%)
			hue-rotate(232deg) brightness(74%) contrast(153%);
	}
